...
import { CheckCircle, XCircle, LogOut, ShieldCheck, Search, Filter, Camera, FileText } from 'lucide-react';
...
export default function WardenPage() {
    ...
    const [activeTab, setActiveTab] = useState<'leaves' | 'photos'>('leaves');
    const [pendingPhotos, setPendingPhotos] = useState<User[]>([]);
    ...
    const handlePhotoStatusUpdate = async (userId: string, status: 'approved' | 'rejected') => {
            ...
            <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
                ...
                <div className="flex gap-4 mb-6 border-b border-slate-200">
                    <button
                        ...
                        className={`pb-3 font-semibold text-sm transition-colors border-b-2 flex items-center gap-2 ${activeTab === 'leaves' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                    >
                    ...
                    <button
                        ...
                        className={`pb-3 font-semibold text-sm transition-colors border-b-2 flex items-center gap-2 ${activeTab === 'photos' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                    >
                        <Camera className="w-4 h-4" />
                        Photo Approvals
                        {pendingPhotos.length > 0 && (
                            <span className="bg-rose-100 text-rose-600 text-xs px-2 py-0.5 rounded-full font-bold ml-1">{pendingPhotos.length}</span>
                        )}
                    </button>
                </div>
                ...
                {activeTab === 'photos' && (
                    <div>
                        <div className="mb-6">
                            <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Photo Approvals</h1>
                            <p className="text-sm text-slate-500 mt-1">Review student profile photos before they are displayed publicly.</p>
                        </div>
                        ...
                        {pendingPhotos.length === 0 ? (
                            <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
                                ...
                                <p className="text-slate-500 text-sm mt-1">There are no pending profile photos to review.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                {pendingPhotos.map(user => (
                                    <div key={user.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm flex flex-col">
                                        ...
                                        <div className="p-4 flex-1 flex flex-col justify-between">
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500">{user.register_number} • Dept: {user.department || 'N/A'}</p>
                                                <p className="text-xs text-slate-500">{user.hostel_block} - Room {user.room_number}</p>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2 mt-4">
                                                <button
                                                    onClick={() => handlePhotoStatusUpdate(user.id, 'approved')}
                                                    className="flex items-center justify-center gap-1.5 py-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-sm font-semibold transition-colors"
                                                >
                                                    <CheckCircle className="w-4 h-4" />
                                                    Approve
                                                </button>
                                                <button
                                                    onClick={() => handlePhotoStatusUpdate(user.id, 'rejected')}
                                                    className="flex items-center justify-center gap-1.5 py-2 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded-lg text-sm font-semibold transition-colors"
                                                >
